...
#define TransferNumber 2
#define TransferLength 784
...
int main(int argc, char *argv[])
{
	 ...
	 */
	test_size = TransferLength; //atoi(argv[2]);
	 ...
	 */
	pthread_create(&tid, NULL, tx_thread, TransferLength); //atoi(argv[1]));

	for (counter = 0; counter < TransferLength; counter++){//atoi(argv[1]); counter++) {
		...
		for (i = 0; i < test_size; i++)
			...
			/*else{
				printf("buffer equal, index = %d, data = %d expected data = %d\n", i,
									rx_proxy_interface_p->buffer[i], (unsigned char)(counter + i));
			}*/
	}
