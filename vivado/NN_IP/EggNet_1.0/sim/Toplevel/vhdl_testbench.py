...
def run_ghdl(filenames, tb_entity, vcd_name="output.vcd"):
    if platform == "linux" or platform == "linux2" or platform == "darwin":
        run_ghdl_linux(filenames=filenames, tb_entity=tb_entity, vcd_name=vcd_name)
    elif platform == "win32":
        run_ghdl_win(filenames=filenames, tb_entity=tb_entity, vcd_name=vcd_name)
    else:
        raise NotImplementedError("Platform is not supported, name is: {}".format(platform))
...
def run_ghdl_linux(filenames, tb_entity, vcd_name="output.vcd"):
    ...
    # Syntax Check and Analysis
    command_s = command_s + ' ' + ' '.join(filenames)
    command_a = command_a + ' ' + ' '.join(filenames)
        ...
        for j in range(kernels.shape[1]):
            for k in range(weights.shape[0]):
                features[i, j, k] = conv_channel(kernels[i, j, :, :, :], weights[k, :, :, :], mav_filter[k],
                                                 msb_detect[k], cnt, data_collecotr)
    cnt.show()
    return features
