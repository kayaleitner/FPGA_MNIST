...
#define new_conv_protofunc_definition(dtype)                                                            \
    ...
    {                                                                                                   \
        ...
        dtype*    data_out = NULL;                                                                      \
        dtype(*array_in)[in_h][in_w][in_ch] = NULL;                                                     \
        dtype(*kernel_in)[fw][in_ch][out_ch] = NULL;                                                    \
        dtype(*array_out)[out_h][out_w][out_ch] = NULL;                                                 \
        debug("data_in   = [%d, %d, %d, %d]", batch, in_h, in_w, in_ch);                                \
        debug("kernel    = [%d, %d, %d, %d]", fh, fw, kin_ch, kout_ch);                                 \
        debug("data_out  = [%d, %d, %d, %d]", batch_out, out_h, out_w, out_ch);                         \
                                                                                                        \
        ...
        *pout_w = out_w;                                                                                \
        *pout_ch = out_ch;                                                                              \
        array_in = (dtype(*)[in_h][in_w][in_ch])data_in;                                                \
        kernel_in = (dtype(*)[fw][in_ch][out_ch])kernel;                                                \
        array_out = (dtype(*)[out_h][out_w][out_ch])data_out;                                           \
                                                                                                        \
        for (int b = 0; b < batch; b++) {                                                               \
            for (int k = 0; k < kout_ch; k++) {                                                         \
                for (int i = 0; i < in_h; i += stride) {                                                \
