...
class GeneralQuantizationTests(unittest.TestCase):
    ...
    def test_next_pow2(self):
        self.assertEqual(1024, EggNet.quant.next_pow2(1023))
        self.assertEqual(64, EggNet.quant.next_pow2(63))
        self.assertEqual(32, EggNet.quant.next_pow2(32))
        self.assertEqual(16, EggNet.quant.next_pow2(16))
        self.assertEqual(4, EggNet.quant.next_pow2(4))
        self.assertEqual(8, EggNet.quant.next_pow2(5))
    ...
    def test_numpy_quant_mul(self):
        ...
        s = -4
        scale = EggNet.quant.fracbits_to_scale(4)
        qkernel = EggNet.quant.quantise_uniform(kernel, scale, 256).astype(np.int8)
        qpatch = EggNet.quant.quantise_uniform(patch, scale, 256).astype(np.int8)
        qr_temp = qkernel * qpatch
        qr = qr_temp.flatten().sum(dtype=np.int8)
        qr2 = qr_temp.flatten().astype(np.int16).sum()
        sr = s + s

        scale_out = EggNet.quant.fracbits_to_scale(8)
        xr_temp = EggNet.quant.dequantise_uniform(qr_temp, scale_out)
        xr = EggNet.quant.dequantise_uniform(qr, scale_out)
        xr3 = EggNet.quant.dequantise_uniform(qr2, scale_out)
        ...
        scale3 = EggNet.quant.fracbits_to_scale(8 - shift)
        yr3 = EggNet.quant.dequantise_uniform(qr3, scale3)
        print(yr3)

    def test_can_mul_overflow(self):
        self.assertTrue(EggNet.quant.can_mul_overflow(16, 8, 16, 8, 8))
        self.assertFalse(EggNet.quant.can_mul_overflow(16, 8, 16, 8, 10))
        self.assertFalse(EggNet.quant.can_mul_overflow(15, 8, 15, 8, 9))
        ...
        for i in range(100):
            ...
            # 2.0 * 3.0 = 6.0
            EggNet.quant.next_pow2(6.0)
            EggNet.quant.next_pow2(6.0)
            EggNet.quant.next_pow2(6.0)
            a = EggNet.quant.quantise_uniform(0.5, scale=1.0 / 16, ncodes=16)
            b = EggNet.quant.quantise_uniform(0.75, scale=1.0 / 16, ncodes=16)

            overflows = EggNet.quant.can_mul_overflow(a=a, a_bits=a_bits, b=b, b_bits=b_bits, out_bits=out_bits)

    def test_can_add_overflow(self):
        # No overflow

        # 3 + 3 = 6
        self.assertFalse(EggNet.quant.can_add_overflow(3, 4, 0, 3, 4, 0, 4))

        # 3 + 0.5
        a = EggNet.quant.quantise_uniform(0.5, scale=1.0 / 16, ncodes=16)
        b = EggNet.quant.quantise_uniform(0.75, scale=1.0 / 16, ncodes=16)
        self.assertTrue(EggNet.quant.can_add_overflow(a, 4, 4, b, 4, 4, 4))
        self.assertFalse(EggNet.quant.can_add_overflow(a, 4, 4, b, 4, 4, 5))
    ...
    def test_quant_kernels(self):
        ...
        bits = 8
        ncodes = 2 ** bits
        qkernel, fracbits = EggNet.quant.quantize_kernels(kernel=kernel, parameter_bits=bits)
        fkernel = EggNet.quant.dequantizse_kernels(qkernel, parameter_bits=bits, frac_bits=fracbits)
    ...
    def test_quant_kernel_precision(self):
        ...
        qk4, mk = EggNet.quant.quantize_kernels(kernel=kernel, parameter_bits=4)
        qi8, mi = EggNet.quant.quantize_conv_activations(input=image, parameter_bits=8)

        qout = EggNet.core.conv2d(data_in=qi8, kernel=qk4)
        mout = mk + mi
...
class QuantTestCase(unittest.TestCase):
    ...
    def test_quant_vec(self):
        import numpy as np
        from EggNet.quant import quantize_vector
...
class FPTestCase(unittest.TestCase):
    ...
    def test_quant_1(self):
        from EggNet.quant import to_fpi, from_fpi
        import random
    ...
    def test_conv(self):
        ...
        kernel = EggNet.make_random_kernel(size=(3, 3, 3, 6))

        qkernel8, km = EggNet.quant.quantize_kernels(kernel, parameter_bits=8)
        qimage8, ki = EggNet.quant.quantize_conv_activations(data_in, parameter_bits=8)
        ...
        qout, om = EggNet.fpi_conv2d(data_in=qimage8, data_in_m=ki,
                                     kernel=qkernel8, kernel_m=km)

        fqout = qout.astype(np.float) / 2 ** om
        fout = EggNet.conv2d(data_in, kernel)
...
class QuantLayerTests(unittest.TestCase):
    ...
    def test_rescale_layer(self):
        ...
        x, m = EggNet.quant.quantize_conv_activations(input, parameter_bits=16)
        scale_layer = EggNet.Layer.RescaleLayer(target_bits=8, axis=(1, 2, 3))
...
class QuantNetworkTests(unittest.TestCase):
    ...
    def test_quant_network_float(self):
        ...
        k1 = EggNet.make_random_kernel(size=(3, 3, 1, 3))
        k2 = EggNet.make_random_kernel(size=(3, 3, 3, 9))

        # Quantize Data
        qk1, m1 = EggNet.quant.quantize_kernels(kernel=k1, parameter_bits=8)
        qk2, m2 = EggNet.quant.quantize_kernels(kernel=k2, parameter_bits=8)
        xq, mx = EggNet.quant.quantize_conv_activations(input=x, parameter_bits=8)

        layers = [
            EggNet.Layer.QuantConv2dLayer(qkernel=qk1, kernel_m=m1),
            EggNet.Layer.RescaleLayer(target_bits=8, source_bits=20, axis=(1, 2, 3)),
            EggNet.Layer.QuantConv2dLayer(qkernel=qk2, kernel_m=m2),
            EggNet.Layer.RescaleLayer(target_bits=8, source_bits=20, axis=(1, 2, 3)),
        ]

        net = EggNet.Network.Network(layers)
        ...
        layers = [
            EggNet.Layer.Conv2dLayer(in_channels=1, out_channels=3, kernel_size=3, use_bias=False,
                                     kernel_init_weights=k1,
                                     bias_init_weights=None, dtype=np.float),
            EggNet.Layer.Conv2dLayer(in_channels=3, out_channels=9, kernel_size=3, use_bias=False,
                                     kernel_init_weights=k2,
                                     bias_init_weights=None, dtype=np.float)
        ]
        float_net = EggNet.Network.Network(layers)
    ...
    def test_quant_network_fpi(self):
        ...
        layers = [
            EggNet.Conv2dLayer(in_channels=1, out_channels=3, kernel_size=3, kernel_init_weights=qk1, use_bias=False),
            EggNet.ShiftLayer(target_bits=8, target_frac_bits=4, source_bits=16, source_frac_bits=8),
            EggNet.Conv2dLayer(in_channels=3, out_channels=9, kernel_size=3, kernel_init_weights=qk2, use_bias=False),
            EggNet.ShiftLayer(target_bits=8, target_frac_bits=4, source_bits=16, source_frac_bits=8),
        ]
